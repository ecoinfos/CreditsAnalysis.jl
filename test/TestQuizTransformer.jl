...
import CreditsAnalysis.LoadQuiz as LQ
...
@testset "CSV file loading test" begin
  ...
  @test LQ.read_csv_to_dataframe(test_csv1) == expected_df_from_csv

  test_csv2 = joinpath(@__DIR__, "test_csv1.xlsx")
  @test_throws ErrorException LQ.read_csv_to_dataframe(test_csv2)
...
@testset "multiple CSVs loading test" begin
  ...
  df_dict1 = LQ.read_multiple_csvs_to_dataframes(test_data_paths1)
  ...
  @test_throws ArgumentError LQ.read_multiple_csvs_to_dataframes(test_data_paths2)
end
...
@testset "DataFrames column titles comparison test" begin
  ...
  @test_logs (:info, "Column titles of dfB are same to dfA's column titles.") (:info, "Column titles of dfC are same to dfA's column titles.") LQ.column_titles_comparison(df_dict1)
  ...
  @test_throws ErrorException LQ.column_titles_comparison(df_dict2)
end
...
@testset "Quiz access data loading test" begin
  ...
  df_dict1 = LQ.read_multiple_access_csvs_to_dict(
    test_data_paths1, class, start_week
  )
  @test typeof(df_dict1) == Dict{Symbol, DataFrame}
  
  test_data_paths2=["test/test_csv1.csv", "test/test_csv1.csv"]
  @test_throws ErrorException LQ.read_multiple_access_csvs_to_dict(
